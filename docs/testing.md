...
Two regression cases cover defects observed while extending the tests:
...
## Real-browser tests: 10 scenarios
...
| UI-02 | Three species, nine outfit combinations, three size settings and loaded images |
...
The current GitHub workflow runs the 17 rule tests and the build. The browser suite is a separate local check unless a browser runtime is explicitly configured in CI. Do not describe local browser results as GitHub Actions results.
